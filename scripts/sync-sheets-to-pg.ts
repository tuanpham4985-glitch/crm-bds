...
function normalizeEmail(email: string | null | undefined, fallback: string): string {
...
async function syncHrm() {
  ...
  for (const nv of rows) {
    ...
    const email = normalizeEmail(nv.email, `${nv.id_nhan_vien}@placeholder.local`);
    try {
...
async function syncTm() {
  ...
  for (const u of users) {
    ...
    const email = normalizeEmail(u.email, `${u.user_id}@placeholder.local`);
    try {
      await prisma.tmUser.upsert({
        ...
        create: {
          ...
          team_id:       u.team_id,
          role:          u.role || 'staff',
          position:      u.position || u.employee_type,
  ...
  for (const t of tasks) {
    ...
    const taskCreateData = (projectId: string | undefined) => ({
      ...
      design_feedback_comment: t.design_feedback_comment,
      project_id:      projectId,
      marketing_project_name: t.marketing_project_name,
      ...
      approver_id:     t.approver_id || undefined,
