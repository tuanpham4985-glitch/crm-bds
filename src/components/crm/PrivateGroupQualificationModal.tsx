...
// Private Group CSKH (task hiện tại) — bản Nhóm-riêng-scoped của
// MembershipQualificationModal.tsx (Campaign CSKH). Cố ý là component RIÊNG,
// không sửa MembershipQualificationModal.tsx: input field ở đây map thẳng
// vào PrivateGroupCustomer (khác endpoint/khác type) — cùng lý do
// MembershipQualificationModal tách khỏi QualificationModal (Customer-global).
import { useState } from 'react';
import { BadgeCheck, Save, X } from 'lucide-react';
import type { PrivateGroupCustomer, MucDoQuanTam } from '@/lib/types';
import { leadQualityRankLabel } from '@/lib/crm-funnel/quality-labels';

export function PrivateGroupQualificationModal({ groupId, relation, onClose, onSaved }: {
  groupId: string;
  relation: PrivateGroupCustomer;
  onClose: () => void;
  onSaved: (relation: PrivateGroupCustomer, message: string) => void;
}) {
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const [form, setForm] = useState({
    san_pham_quan_tam: relation.san_pham_quan_tam || '', nhu_cau: relation.nhu_cau || '',
    ngan_sach_min: relation.ngan_sach_min ?? '', ngan_sach_max: relation.ngan_sach_max ?? '',
    muc_dich: relation.muc_dich || '', thoi_gian_du_kien: relation.thoi_gian_du_kien || 'Chưa xác định',
    phuong_an_tai_chinh: relation.phuong_an_tai_chinh || '', khu_vuc_yeu_cau: relation.khu_vuc_yeu_cau || '',
    muc_do_quan_tam: relation.muc_do_quan_tam || 'Chưa xác định' as MucDoQuanTam,
    hanh_dong_tiep_theo: relation.hanh_dong_tiep_theo || '',
  });
  ...
  async function save() {
    ...
    try {
      const response = await fetch(`/api/private-groups/${groupId}/customers/${relation.id}/qualification`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...form, idempotency_key: crypto.randomUUID() }),
      });
      const result = await response.json();
      if (!result.success) throw new Error(result.error);
      onSaved({ ...result.data, customer: relation.customer }, `Điểm tiềm năng ${result.score.score}/100 · ${leadQualityRankLabel(result.score.rank)}`);
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Không thể lưu qualification'); }
    finally { setSaving(false); }
  }

  return <div className="modal-overlay" onClick={onClose}><div className="modal-content" style={{ maxWidth: 760 }} onClick={event => event.stopPropagation()}><div className="modal-header"><h3 className="modal-title"><BadgeCheck size={18} /> Đánh giá: {relation.customer?.ten_KH || relation.customer_id}</h3><button className="btn btn-ghost btn-icon" onClick={onClose}><X size={18} /></button></div><div className="modal-body">
    ...
    <div style={{ background: '#eff6ff', color: '#1d4ed8', padding: 10, borderRadius: 7, fontSize: 12, marginTop: 12 }}>Điểm và Xếp hạng tiềm năng do server tự tính, riêng cho Nhóm riêng này. Sale không thể nhập hoặc sửa điểm trực tiếp.</div>
