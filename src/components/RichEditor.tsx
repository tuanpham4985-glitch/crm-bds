...
import { Bold, Italic, Underline, Type, Smile, RotateCcw, List, Paperclip } from 'lucide-react';
...
// Inject editor CSS into <head> once — keeps it out of the flex/DOM layout
const CSS_ID = 'rich-editor-styles';
if (typeof window !== 'undefined' && !document.getElementById(CSS_ID)) {
  const el = document.createElement('style');
  el.id = CSS_ID;
  el.textContent = `
    .rich-editor-body ul { padding-left: 24px; margin: 4px 0; }
    .rich-editor-body li { list-style: disc; margin: 2px 0; }
    .rich-editor-body[contenteditable]:empty::before {
...
interface Props {
  ...
  placeholder?: string;
  minHeight?: number;
  /** Lấp đầy chiều cao container cha qua flex */
  fillHeight?: boolean;
}
...
export default function RichEditor({
  ...
  // Sync when disabled (read-only preview)
  useEffect(() => {
  ...
  // All toolbar actions use onMouseDown+preventDefault to keep editor focus
  ...
  const insertEmoji = (em: string) => (e: React.MouseEvent) => {
    e.preventDefault();
    if (disabled) return;
    editorRef.current?.focus();
    // eslint-disable-next-line @typescript-eslint/no-deprecated
  ...
  const onDragEnter = (e: React.DragEvent) => {
    ...
    if ([...e.dataTransfer.items].some(i => ALLOWED.includes(i.type))) setDragging(true);
  };
  ...
  const onDrop = (e: React.DragEvent) => {
    ...
    setDragging(false);
    if (disabled || !onDropRef.current) return;
    const files = [...e.dataTransfer.files].filter(f => ALLOWED.includes(f.type));
  ...
  // Outer wrapper: flex column when fillHeight so editor can use flex:1
  const wrapStyle: React.CSSProperties = fillHeight
    ? { display:'flex', flexDirection:'column', height:'100%', minHeight:0, position:'relative' }
    : { position:'relative' };

  // Editor area wrapper: flex:1 when fillHeight
  const areaWrapStyle: React.CSSProperties = fillHeight
  ...
  return (
    <div style={wrapStyle}>
      ...
      <div style={{
        ...
        border:'1px solid var(--border-light,#e2e8f0)', borderBottom:'none',
        borderRadius:'8px 8px 0 0', userSelect:'none', boxSizing:'border-box',
        flexShrink: 0,
      }}>
        ...
        <div style={sep}/>
        <button style={iconBtn()} title="Danh sách" onMouseDown={cmd('insertUnorderedList')}><List size={13}/></button>
        <div style={sep}/>
        {COLORS.map(c => (
          <button key={c.value} title={c.label} onMouseDown={cmd('foreColor', c.value)}
            style={{ width:20, height:20, borderRadius:4, padding:0, border:'2px solid rgba(0,0,0,0.1)',
        ...
        <select disabled={disabled} defaultValue="" onMouseDown={e=>e.stopPropagation()}
          onChange={e=>{ if(e.target.value){ applySize(e.target.value); e.target.value=''; }}}
          style={{ fontSize:12, border:'1px solid var(--border-light,#e2e8f0)', borderRadius:4,
            padding:'2px 4px', background:'#fff', cursor: disabled?'not-allowed':'pointer',
            color:'var(--text-body,#374151)' }}>
          <option value="" disabled>Cỡ chữ</option>
          {SIZES.map(s=><option key={s.value} value={s.value}>{s.label}</option>)}
        </select>
        <div style={sep}/>
        <button style={iconBtn(showEmoji)} title="Chèn emoji"
          onMouseDown={e=>{ e.preventDefault(); setShowEmoji(v=>!v); }}>
          <Smile size={13}/>
        </button>
        <button style={{...iconBtn(), marginLeft:'auto'}} title="Xóa định dạng" onMouseDown={cmd('removeFormat')}>
          <RotateCcw size={12}/>
        </button>
      </div>

      {/* Editor area */}
      <div style={areaWrapStyle}>
        <div
          ref={editorRef}
        ...
        {dragging && (
          <div style={{
            position:'absolute', inset:0, borderRadius:'0 0 8px 8px',
            background:'rgba(99,102,241,0.08)', border:'2px dashed #6366f1',
            display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center',
            gap:8, pointerEvents:'none',
          }}>
            <Paperclip size={28} color="#6366f1"/>
            <span style={{ fontSize:14, fontWeight:600, color:'#4f46e5' }}>Thả file để đính kèm</span>
            <span style={{ fontSize:12, color:'#818cf8' }}>Ảnh (JPG, PNG, GIF) và PDF</span>
          </div>
        )}
      </div>
      ...
      {showEmoji && (
        <>
          ...
          <div style={{
            ...
            {EMOJIS.map(em=>(
              <button key={em} onMouseDown={insertEmoji(em)}
                style={{ fontSize:20, width:36, height:36, border:'none', borderRadius:7,
