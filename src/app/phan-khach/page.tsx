...
export default function PhanKhachPage() {
  ...
  const stats = useMemo(() => ({
    ...
    notCalled: customers.filter(item => (item.trang_thai_cham_soc || 'Chưa gọi') === 'Chưa gọi').length,
    pending: customers.filter(item => ['Gọi lại', 'Không nghe máy'].includes(item.trang_thai_cham_soc || '')).length,
  ...
  async function saveInteraction() {
    ...
    try {
      ...
      setNotice({ type: data.warning ? 'warn' : 'ok', text: data.warning || (data.handoff ? `Khách đã quan tâm, đã bàn giao ngay cho ${data.data.sale_nhan_khach || 'hàng chờ quản lý'}.` : 'Đã lưu kết quả chăm sóc và cập nhật funnel.') });
  ...
  return <div>
    ...
    {selectedProject && <><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(130px,1fr))', gap: 10, marginBottom: 16 }}>{[
      ['Tổng data', stats.total, '#334155'], ['Chưa phân', stats.unassigned, '#7c3aed'], ['Chưa gọi', stats.notCalled, '#64748b'], ['Cần chăm sóc', stats.pending, '#2563eb'], ['Quá lịch', stats.overdue, '#dc2626'], ['Quan tâm', stats.interested, '#059669'], ['Chờ Sale nhận', stats.handoff, '#d97706'], ['Đã nhận', stats.accepted, '#16a34a'],
...
function CustomerTable({ customers, loading, busyId, canManage, userName, telesales, sales, onAssign, onInteraction, onQualification, onHandoff, onHistory }: {
  ...
  return <div className="card" style={{ padding: 0, overflow: 'hidden' }}><div className="table-wrapper" style={{ overflowX: 'auto' }}><table className="data-table" style={{ minWidth: 1380 }}><thead><tr><th>Khách hàng</th><th>Dự án / nhu cầu</th><th>Telesale phụ trách</th><th>Tiến độ</th><th>Lịch tiếp theo</th><th>Bàn giao Sale</th><th style={{ textAlign: 'right' }}>Thao tác</th></tr></thead><tbody>{customers.map(customer => {
    ...
    return <tr key={customer.id_khach_hang} style={isOverdue(customer.ngay_lien_he_tiep) ? { background: '#fff7f7' } : undefined}>
      ...
      <td><div style={{ display: 'flex', justifyContent: 'flex-end', gap: 6, flexWrap: 'wrap' }}>{(assignee || canManage) && customer.trang_thai_ban_giao !== 'Đã nhận' && <button className="btn btn-primary btn-sm" title="Cập nhật kết quả chăm sóc khách hàng" onClick={() => onInteraction(customer)}><Phone size={13} /> Chăm sóc</button>}{(assignee || canManage) && <button className="btn btn-secondary btn-sm" title="Đánh giá nhu cầu và mức độ tiềm năng của khách hàng" onClick={() => onQualification(customer)}><BadgeCheck size={13} /> Đánh giá</button>}{receiver && customer.trang_thai_ban_giao === 'Chờ xác nhận' && <><button className="btn btn-primary btn-sm" onClick={() => void onHandoff(customer, 'accept')}><UserCheck size={13} /> Nhận khách</button><button className="btn btn-secondary btn-sm" onClick={() => void onHandoff(customer, 'reject')}><X size={13} /> Từ chối</button></>}<button className="btn btn-secondary btn-sm" title="Xem lịch sử chăm sóc" onClick={() => onHistory(customer)}><History size={13} /> Lịch sử</button></div></td>
    </tr>;
  })}</tbody></table></div></div>;
}
