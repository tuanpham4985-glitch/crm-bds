...
function buildHtml(row: RowWithEmail, thang: number, nam: number): string {
  ...
  if (isKD) {
    if (row.cong_thuc_te)       congRows.push(tr('Công thực tế',           String(row.cong_thuc_te)));
    if (row.cong_tinh_luong)    congRows.push(tr('Công tính lương',         String(row.cong_tinh_luong)));
    if (row.cong_tv)            congRows.push(tr('Công thử việc (TV)',      String(row.cong_tv)));
    if (row.cong_ct)            congRows.push(tr('Công chính thức (CT)',    String(row.cong_ct)));
    if (row.luong_tv)           congRows.push(tr('Lương thử việc',           fmtM(row.luong_tv)));
    if (row.luong_ct)           congRows.push(tr('Lương chính thức',         fmtM(row.luong_ct)));
  } else {
    if (row.cong_thuc_te)       congRows.push(tr('Giờ công CT',             String(row.cong_thuc_te)));
    if (row.cong_tv)            congRows.push(tr('Giờ công TV',             String(row.cong_tv)));
    if (row.luong_ct)           congRows.push(tr('Lương CT/tháng',           fmtM(row.luong_ct)));
    if (row.luong_tv)           congRows.push(tr('Lương TV/tháng',           fmtM(row.luong_tv)));
  }
  ...
  if (row.luong_dong_bhxh)    dedRows.push(trGray('Lương đóng BHXH (cơ sở)',      fmtM(row.luong_dong_bhxh)));
  ...
  if ((row.thu_nhap_tinh_thue ?? 0) > 0)
                              dedRows.push(trGray('Thu nhập tính thuế',            fmtM(row.thu_nhap_tinh_thue)));
  else                        dedRows.push(trGray('Thu nhập tính thuế',            'Miễn thuế'));
...
.wrap{max-width:620px;margin:20px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.1)}
.hdr{background:linear-gradient(135deg,#1e3a5f,#2d5a8e);color:#fff;padding:24px 28px}
.hdr-co{font-size:15px;font-weight:800;letter-spacing:.06em;margin:0 0 2px}
.hdr-sub{margin:0;font-size:12px;opacity:.75}
.body{padding:24px 28px}
.greeting{font-size:14px;margin:0 0 16px;color:#374151}
.sec{margin-bottom:18px}
.sec-hdr{font-size:11px;font-weight:700;color:#fff;background:#1e3a5f;padding:6px 12px;border-radius:4px;margin:0 0 1px;text-transform:uppercase;letter-spacing:.05em}
.info-grid{display:grid;grid-template-columns:1fr 1fr;gap:0;border:1px solid #e5e7eb;border-radius:0 0 6px 6px;overflow:hidden;font-size:13px}
.info-item{padding:8px 12px;border-bottom:1px solid #f3f4f6;border-right:1px solid #f3f4f6}
.info-item:nth-child(even){border-right:none}
.info-lbl{color:#9ca3af;font-size:10px;text-transform:uppercase;letter-spacing:.04em;margin-bottom:2px}
table{width:100%;border-collapse:collapse;font-size:13px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 6px 6px;overflow:hidden}
...
.tr-i td:first-child{padding-left:24px;color:#6b7280;font-size:12px}
.tr-red td:last-child{color:#dc2626}
.tr-blue td:last-child{color:#2563eb}
.tr-gray td{color:#9ca3af!important;font-size:12px}
.tr-total td{background:#d1fae5;color:#065f46!important;font-weight:700!important;font-size:13px;padding:9px 12px;border:none}
.tr-deduct td{background:#fee2e2;color:#991b1b!important;font-weight:700!important;font-size:13px;padding:9px 12px;border:none}
.thuc-linh{background:#064e3b;border-radius:8px;text-align:center;padding:20px;margin-top:20px}
.thuc-linh .lbl{color:rgba(255,255,255,.7);font-size:11px;text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px}
.thuc-linh .amt{color:#fff;font-size:28px;font-weight:800;letter-spacing:-.01em}
.note{font-size:12px;color:#6b7280;background:#f9fafb;padding:10px 14px;border-radius:6px;margin-top:14px;line-height:1.6;border:1px solid #e5e7eb}
.footer{background:#f3f4f6;padding:12px 28px;font-size:11px;color:#9ca3af;text-align:center;border-top:1px solid #e5e7eb}
</style></head>
<body>
<div class="wrap">
  <div class="hdr">
    <div class="hdr-co">CÔNG TY TNHH VICTORY HOLDINGS VIỆT NAM</div>
    <p class="hdr-sub">PHIẾU LƯƠNG THÁNG ${thang}/${nam} &mdash; ${isKD ? 'KHỐI KINH DOANH' : 'KHỐI BACK OFFICE'}</p>
  </div>
  <div class="body">
    <p class="greeting">Kính gửi <strong>${row.ho_ten}</strong>,<br>
    Dưới đây là thông tin lương tháng <strong>${thang}/${nam}</strong> của bạn:</p>

    <!-- I. THÔNG TIN NHÂN SỰ -->
    <div class="sec">
      <div class="sec-hdr">I. Thông tin nhân sự</div>
      <div class="info-grid">
        <div class="info-item"><div class="info-lbl">Mã nhân viên</div><strong>${row.id_nhan_vien}</strong></div>
        <div class="info-item"><div class="info-lbl">Họ và tên</div><strong>${row.ho_ten}</strong></div>
        ...
        <div class="info-item"><div class="info-lbl">Loại hợp đồng</div><strong>${row.ct_tv || '—'}</strong></div>
    ...
    <!-- II. CÔNG THÁNG -->
    <div class="sec">
      <div class="sec-hdr">II. Công tháng</div>
      <table>${congRows.join('')}</table>
    </div>` : ''}

    <!-- III. THU NHẬP -->
    <div class="sec">
      <div class="sec-hdr">III. Thu nhập</div>
      <table>
        ${incRows.join('')}
    ...
    <!-- IV. KHẤU TRỪ -->
    <div class="sec">
      <div class="sec-hdr">IV. Khấu trừ</div>
      <table>
        ${dedRows.join('')}
  ...
  <div class="footer">&copy; ${new Date().getFullYear()} Công ty TNHH Victory Holdings Việt Nam &middot; victoryholdings.com.vn</div>
</div>
</body></html>`;
}
